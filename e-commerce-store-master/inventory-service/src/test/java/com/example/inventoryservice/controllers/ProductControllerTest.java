...
class ProductControllerTest {
    ...
    @Test
    void getByCategory_WhenCategoryExistsAndStockAvailable_ShouldReturnSortedProducts() {
        given()
                .spec(this.requestSpec)
                .queryParam("sortBy", "price")
                .when()
                .get(API_V_1_PRODUCTS + "/category/Electronics")
                .then()
                .log().ifValidationFails()
                .statusCode(200)
                .body("size()", is(3)); // assuming 3 products in Electronics with quantity > 0 in setup.sql
    }

    @Test
    void updateProduct_WhenProductExists_ShouldUpdateSuccessfully() {
        var updatedProduct = ProductCreateDto.builder()
                .name("Updated iPhone 13 Pro")
                .price(BigDecimal.valueOf(899.00f))
        ...
        given()
                ...
                .body(updatedProduct)
                .when()
                .put(API_V_1_PRODUCTS + "/update/1")
                .then()
                .log().ifValidationFails()
                .statusCode(200)
                .body("id", is(1))
                .body("name", equalTo("Updated iPhone 13 Pro"))
                .body("price", is(899.00f));
    }
